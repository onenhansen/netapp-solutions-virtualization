...
= Learn about enhancing virtualized IT environments with ONTAP
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Virtualization optimizes IT environments, enabling efficient resource use and flexibility while supporting modern applications like AI systems. Learn about its impact on network, compute, and storage infrastructure, and how NetApp ONTAP enhances virtualized systems.
...
image:virt-overview-001.png["Diagram of the virtualization components for compute, network, and storage", width=40%]
