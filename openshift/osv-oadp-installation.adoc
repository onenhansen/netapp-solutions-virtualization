...
= Install the Red Hat OpenShift API for Data Protection (OADP) operator
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Install the OpenShift API for Data Protection (OADP) Operator to enable backup and restore capabilities for VMs in OpenShift Virtualization. This procedure includes deploying the OADP Operator from the OpenShift Operator Hub, configuring Velero to use NetApp ONTAP S3 or StorageGRID as the backup target, and setting up the necessary secrets and backup locations.
...
image:redhat-openshift-oadp-install-006.png[Create Secret for S3 user credentials using CLI]
...
// NetApp Solutions restructuring (jul 2025) - renamed from containers/rh-os-n_use_case_openshift_virtualization_dataprotection_installation.adoc
