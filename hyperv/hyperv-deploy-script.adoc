...
---
= Migrate VMs to Microsoft Hyper-V using the PowerShell script
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use the PowerShell script to migrate VMs from VMware vSphere to Microsoft Hyper-V using FlexClone technology. The script streamlines the migration process  by connecting to vCenter and ONTAP clusters, creating snapshots, converting VMDKs to VHDXs, and configuring VMs on Hyper-V.
