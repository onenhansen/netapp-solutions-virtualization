...
---
= Deployment guidelines for Microsoft Hyper-V with ONTAP storage systems
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
To ensure optimal performance and reliability when deploying Microsoft Hyper-V with ONTAP storage, consider factors such as workload compatibility, storage sizing, and VM resource allocation. Compatibility checks should include OS versions, applications, databases, and any existing customizations to ensure smooth operation within the Hyper-V environment.
...
+
image:hyperv-deploy-003.png[Image of the SMB colume settings]
...
+
image:hyperv-deploy-004.png[Image of the NTFS data volume settings]
+
...
+
image:hyperv-deploy-005.png[IMage of the NTFS permissions settings]
...
+
image:hyperv-deploy-006.png[Image of the General Properties page for Hyper-V LUN creation]
...
+
image:hyperv-deploy-007.png[Image of the Add to Cluster Shared Volumes interface]
