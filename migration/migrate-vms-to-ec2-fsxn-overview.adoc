...
= Learn about migrating VMs to Amazon EC2 using Amazon FSx for ONTAP
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Amazon FSx for NetApp ONTAP makes it easy to migrate VMware vSphere VMs to Amazon EC2, preserving important storage features like snapshots, thin provisioning, and replication. It supports familiar tools and protocols, simplifying the migration process and reducing cost and complexity.
...
// NetApp Solutions restructuring (jul 2025) - renamed from vmware/migrate-vms-to-ec2-fsxn.adoc
