...
= Add an iSCSI datastore as supplemental storage for management domain using ONTAP tools for VMware vSphere
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
+
image:vmware-vcf-asa-005.png[Choose to create new port group]
+
{nbsp}
...
+
image:vmware-vcf-asa-006.png[Fill out VLAN ID]
+
{nbsp}
...
+
image:vmware-vcf-asa-027.png[DPG - edit settings]
+
{nbsp}
...
+
image:vmware-vcf-asa-028.png[move uplink2 to unused]

. Repeat this step for the second iSCSI port group. However, this time move *uplink1* down to *Unused uplinks*.
+
image:vmware-vcf-asa-029.png[move uplink1 to unused]
====
...
+
image:vmware-vcf-asa-007.png[Start add networking wizard]
+
{nbsp}
...
+
image:vmware-vcf-asa-009.png[Choose target port group]
+
{nbsp}
. On the *Port properties* page keep the defaults and click on *Next* to continue.
+
image:vmware-vcf-asa-010.png[VMkernel port properties]
+
{nbsp}
