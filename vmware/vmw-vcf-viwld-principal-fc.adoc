...
= Use an FC-based VMFS datastore on ONTAP as principal storage for VI workload domain
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
In this use case we outline the procedure to set up a Fibre Channel (FC) VMFS datastore on ONTAP as the primary storage solution for a VMware Cloud Foundation (VCF) Virtual Infrastructure (VI) workload domain. This procedure summarizes the required components, configuration steps, and deployment process.
